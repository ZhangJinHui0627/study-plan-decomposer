...
- 自然语言学习计划解析与任务拆分
- 任务详情展开双通道设计（卡片详情展开可并排呈现“开始专注”与“标记完成”独立入口）
- 右滑任务卡片无条件统一进入多选模式（彻底移除原“允许手动打卡”开关以简化操作并避免误触打卡）
- 任务数据可视化与完成率、专注时长和阅读页数统计
- 全局弹窗样式标准化（统一采用 24dp 呼吸内边距、圆角卡片背景，并使用胶囊式 TextView 操作按钮，删除/清空等危险操作配备红色高亮）
- 专注计时运行时的 16 秒慢速呼吸蓝青屏幕物理流光边框（暂停或结束时外框优雅隐退）
- 个人中心头像默认“添加图片”文字垂直水平几何中心对齐
- 本地学习提醒与系统日历双向日程同步管理
- 倒计时、正计时和溢出时间记录
